...
function createWebpackConfig({isServer=false, env='development'}) {
  ...
  const projName=require(APP_PATH+'/package.json').name || 'demo1'
  base.output.chunkCallbackName='webpackChunk_'+projName
  base.output.jsonpFunction='webpackJsonp_'+projName
  if(!IS_DEV) {
    base.output.path=APP_PATH+'/dist'
    base.output.publicPath='/assets/app/'
  }
  const conf=Object.assign(Object.assign({}, base), webpackConfig)
  conf.devtool=IS_DEV? 'source-map': 'none'
...
function resolveAssets(assets) {
  ...
  for(let x of assets) {
    if(/\.js$/.test(x)) ret.js.push(x)
    if(/\.css$/.test(x)) ret.css.push(x)
  }
  return ret
}
  ...
  multiCompiler.hooks.done.tap('done', stats=>{
    ...
    if(errors.length) {
      ...
      const ret={
        ...
        client: resolveAssets(clientStats.entrypoints.main.assets),
      }
